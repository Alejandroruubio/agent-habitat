      ...
      <div className="absolute top-0 left-0 right-80 z-10 px-6 py-3 flex items-center justify-between glass">
        <div className="flex items-center gap-3">
          <div className={`w-2 h-2 rounded-full ${
            alertLevel === 'red' ? 'bg-destructive' : alertLevel === 'yellow' ? 'bg-[hsl(var(--status-speaking))]' : 'bg-[hsl(var(--status-online))]'
          } status-dot`} />
        ...
        <div className="flex items-center gap-4 text-xs text-muted-foreground font-mono">
          <span>4 Agents</span>
          <span>•</span>
          <span>System {alertLevel === 'red' ? 'ALERT' : alertLevel === 'yellow' ? 'Warning' : 'Nominal'}</span>
        </div>
      </div>
